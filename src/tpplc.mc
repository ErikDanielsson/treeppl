...
let mcmcLightweightOptions : OptParser (Type -> Loader -> (Loader, InferMethod)) =
  let mk = lam debugIterations. lam thinPeriod. lam incrementalPrinting. lam iterations. lam globalProb. lam driftKernel. lam driftScale. lam cps. lam align. lam outputType. lam loader.
  ...
  let incrementalPrinting = optFlag
    ...
    let res = optApply (optApply (optApply (optApply (optMap5 mk debugIterations thinPeriod incrementalPrinting _particles _mcmcLightweightGlobalProb) _driftKernel) _driftScale) _cps) _align in
